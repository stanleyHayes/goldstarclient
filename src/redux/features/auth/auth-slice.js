...
const verifyAccount = createAsyncThunk(
    ...
    async (
        ...
        try {
            setSubmitting(true);
            const response = await AUTH_API.verifyAccount(values, token);
...
const getProfile = createAsyncThunk('auth/getProfile',
    async ({token, navigate}, {rejectWithValue}) => {
        try {
            const response = await AUTH_API.getProfile(token);
            return response.data;
        } catch (e) {
            const {message} = e.response.data;
...
const updateProfile = createAsyncThunk('auth/updateProfile',
    async ({token, resetForm, setSubmitting, ...rest}, {rejectWithValue}) => {
        try {
            const response = await AUTH_API.updateProfile(token, {...rest});
            resetForm();
            setSubmitting(false);
            localStorage.setItem(CONSTANTS.GOLD_STAR_AUTH_TOKEN, JSON.stringify(response.data.token));
            localStorage.setItem(CONSTANTS.GOLD_STAR_AUTH_DATA, JSON.stringify(response.data.data));
...
const verifyOTP = createAsyncThunk('auth/verifyOTP',
    async ({values, token, navigate, resetForm, showMessage}, {rejectWithValue}) => {
        try {
            const response = await AUTH_API.verifyOTP(values, token);
            localStorage.setItem(CONSTANTS.GOLD_STAR_AUTH_TOKEN, JSON.stringify(response.data.token));
            localStorage.setItem(CONSTANTS.GOLD_STAR_AUTH_DATA, JSON.stringify(response.data.data));
            navigate(`/`);
            resetForm();
...
const resendOTP = createAsyncThunk('auth/resendOTP',
    async ({
        ...
        try {
            const response = await AUTH_API.resendOTP(values);
            navigate(`/auth/otp/${response.data.token}/verify`);
            resetForm();
            showMessage(response.data.message, {variant: 'success'});
            localStorage.setItem(CONSTANTS.GOLD_STAR_AUTH_TOKEN, JSON.stringify(response.data.token));
